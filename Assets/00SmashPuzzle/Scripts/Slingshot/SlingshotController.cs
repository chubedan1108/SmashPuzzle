...
public class SlingshotController : MonoBehaviour
{
    ...
    private readonly float _gravity = Mathf.Abs(Physics.gravity.y);
    private void Awake()
    {
    ...
    private void Start()
    {
        ...
        {
            if (ObjectPool.Instance.GetObject(PoolType.Bullet).GetComponent<Bullet>() == null)
            {
                Debug.Log("Bullet prefab is set in the ObjectPool.");
            }
    ...
    {
        //bullet = ObjectPool.Instance.GetObject(PoolType.Bullet).GetComponent<Bullet>();
        //SetupBullet();
        var prefab = Instantiate(bulletPrefab, firePoint.position, firePoint.rotation, transform);
        Bullet bull = prefab.GetComponent<Bullet>();
        bull.gameObject.SetActive(true);
        Vector3 caculateVelocity = CalculateLaunchVelocity(firePoint.position, target);
        bull.Launch(caculateVelocity);
    }
